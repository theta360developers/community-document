...
== Additional Live Streaming Guides

This is part of a set of live streaming guides and videos that 
are available on the 
http://livestreaming.theta360.guide/[RICOH THETA live streaming site].
...
requires a USB OTG Ethernet adapter plugged into the USB port. In most use cases, the THETA V will not 
charge while using Ethernet. The battery will drain. Community members have 
...
The most common method of streaming the THETA V and Z1 is with a USB cable connected to a
Windows or Mac computer.  However, it is possible to stream using WiFi.  All the methods
below use free software.

.Table THETA V and Z1 Streaming Method
|===
|Method |Software Needed |Note 

|USB to laptop then to YouTube over Internet
|Windows 10 users need a driver. See http://livestreaming.theta360.guide/[live streaming site]. 
...
|https://pluginstore.theta360.com/plugins/com.theta360.cloudstreaming/[Wireless Live Streaming Plug-in]
...
|https://pluginstore.theta360.com/plugins/com.sciencearts.rtspstreaming/[THETA RTSP Streaming]
|Max resolution is 3840x1920. Connect over Wi-Fi to a laptop running video player software.

|===

NOTE: The THETA V can stream for about 8 hours.  It consumes slightly more power while streaming than
the the charge provided.  With USB-C, the Z1 can stream indefinitely. Heat may be a problem.

== RICOH THETA V and Z1 Software 

=== Wi-Fi RTMP Streaming Direct to YouTube icon:puzzle-piece[2x]

With the 
https://pluginstore.theta360.com/plugins/com.theta360.cloudstreaming/[Wireless Live Streaming] plug-in, you can stream the THETA V and Z1 through a Wi-Fi router or a mobile phone used as a hotspot to YouTube 
Live Events in 360 or other
live event sites that use RTMP. Most
cloud services like YouTube use
RTMP.  As of July 8, 2020, Facebook
live events, which also uses RTMP, no longer works with 
360 live events.  We hope that
Facebook will add this feature
back in the future. A step-by-step guide is available 
https://theta360developers.github.io/wireless-plugin-guide/[here].
...
=== Wi-Fi RTSP Streaming Direct to Computer icon:puzzle-piece[2x]

RTSP is used to stream your camera
live video to a desktop application
running on your laptop or headset.
You can see the RTSP stream in
software such as VLC, ffplay, or
GoPro VR Player 3.

Get the 
https://pluginstore.theta360.com/plugins/com.sciencearts.rtspstreaming/[THETA RTSP Streaming plug-in] from the
THETA Store.  It is free to download and
use.

image::img/livestreaming/rtsp/rtsp-plug-in.png[role="thumb" title="RTSP Plug-in installation from store"]

Press install.

image::img/livestreaming/rtsp/rtsp-install.png[role="thumb" title="RTSP Plug-in install into camera"]
...
image::img/livestreaming/rtsp/rtsp-activate.png[role="thumb" title="RTSP Plug-in activation"]

With the plug-in activated, you can start the plug-in by pressing and holding the mode button of the camera.

To use the plug-in, you must connect the THETA V to your home router with Client Mode and
...
When the THETA V is running the THETA RTSP Streaming plug-in, it will display
the LEDs shown below.

image::img/livestreaming/rtsp/rtsp-camera-led.png[role="thumb" title="THETA V LEDs when RTSP Plug-in is running"]

Test with https://www.videolan.org/vlc/index.html[VLC].
...
image::img/livestreaming/rtsp/rtsp-vlc-open.png[role="thumb" title="Open network stream on VLC"]

* Change IP address to your THETA camera. Example rtsp://192.168.2.101:8554/live?resolution=640x320 for the network URL
* Press “Play”


image::img/livestreaming/rtsp/rtsp-network-address.png[role="thumb" title="Set network URL in VLC to the IP address of your camera."]

An equirectangular view of the video feed from the THETA will appear in VLC.

image::img/livestreaming/rtsp/rtsp-vlc-view.png[role="thumb" title="View a live equirectangular video stream from the RICOH THETA with Wi-Fi in VLC"]

No wires needed!

You can adjust the video resolution to 1920x960 or 3840x1920.

image::img/livestreaming/rtsp/rtsp-3840x1920.png[role="thumb" title="Configure VLC for 3840x1920 live streaming over Wi-Fi"]

The video looked great with a high framerate and low latency.

image::img/livestreaming/rtsp/rtsp-3840x1920-view.png[role="thumb" title="View 3840x1920 live video over Wi-Fi"]

At 3840x1920 network traffic was 300 KiB/second.


image::img/livestreaming/rtsp/rtsp-network-traffic.png[role="thumb" title="3840x1920 requires 300 KiB/second bandwidth on Wi-Fi"]

=== Ethernet RTSP Streaming Direct to Computer icon:puzzle-piece[2x]

The RTSP plug-in works with Ethernet.  To test the plug-in, I
used an USB to Ethernet adapter and Y-splitter for the adapter to
also test if I can charge the camera while it is streaming over
Ethernet.

image::img/livestreaming/rtsp/rtsp-ethernet-rig.png[role="thumb" title="Using RTSP plug-in with Ethernet"]

With my computer and the THETA V on the same subnet, I found the
IP address of the THETA V by running a script on my computer.
The script is part of my 
https://github.com/codetricity/theta-client-mode[theta-client-mode] tests on GitHub.

image::img/livestreaming/rtsp/find-theta.png[role="thumb" title="Find IP address of THETA V"]

To establish a baseline prior to the RTSP Ethernet test, I used the same script
to check the camera state. 

image::img/livestreaming/rtsp/camera-state-initial.png[role="thumb" title="Camera state prior to RTSP test"]

The state looks very promising.  The camera indicates that it's charging.
Battery level is at 84%.

I started the plug-in by pressing and holding the lower mode button
of the THETA V.

I opened a network stream with vlc.


image::img/livestreaming/rtsp/ethernet-rtsp-vlc-config.png[role="thumb" title="Camera state prior to RTSP test"]

image::img/livestreaming/rtsp/vlc-working.png[role="thumb" title="VLC works with smooth video"]

Latency is around 2 seconds with my test resolution of 640x320.

image::img/livestreaming/rtsp/latency.png[role="thumb" title="Latency between camera and VLC over Ethernet is about 2 seconds"]

At this point, I've been streaming over Ethernet for 40 minutes continuously.

After an hour of streaming over Ethernet, the battery did drop
a little.

image::img/livestreaming/rtsp/camera-state-end.png[role="thumb" title="Camera state after one hour of RTSP streaming"]

==== GoPro VR Player 3

To get a 360 view of the live stream, you can use GoPro VR Player 3.

image::img/livestreaming/rtsp/rtsp_gopro_vr_player_3.png[role="thumb" title="GoPro VR Player 3"]

In my tests on July 12, 2020, I was only able to use GoPro VR Player 3 on
Mac.  When I tried to use the player on Windows with 
THETA RTSP plug-in, the GoPro VR Player would crash.  This is a low-resolution
test.
...
for more information. Not all USB OTG Ethernet adapters will work.

Community member Kev091190 has reported success using an USB splitter with the 
THETA V.  Oppkey staff tested a different USB OTG Ethernet adapter and USB splitter
...
To test the charging, Oppkey connected a current meter to the Y-splitter.  In the first
test, we looked at the charge prior to streaming the camera with Ethernet.

image::img/livestreaming/ethernet/before-plug-in.png[role="thumb" title="setup before starting plug-in"]

Prior to the plug-in starting, the camera drew 0.41Amps. 

image::img/livestreaming/ethernet/before-plug-in-meter.png[role="thumb" title="Camera drawing 0.41A before plug-in starts"]

After the plug-in started, the camera current draw did not change.  It remained at 0.41amps.

image::img/livestreaming/ethernet/after-plug-in-meter.png[role="thumb" title="Camera drawing 0.41A after plug-in starts"]

I connected the camera to a 2amp power source and streamed it for three
hours. The battery charge increased from 75% to 84% during the streaming
session.  The camera body was warm, but not hot at the end of the test.  In the
next test, I streamed it for 17 hours continuously.  The camera may
be able to stream indefinitely.

image::img/livestreaming/ethernet/rig-2amp-power.png[role="thumb" title="Camera attached to a 2amp power source"]

image::img/livestreaming/ethernet/streaming-test-charging.png[role="thumb" title="THETA V charge increased after 3 hours of live streaming over Ethernet"]

As this was my first time using an USB Y-splitter to power the THETA V while it
is connected to a USB OTG device, it took me a long time to figure out which
end of the connector to put into what port.  

This is the adapter that I am using.  I think the adapters are largely similar.
Make sure that you get one that indicates it is used to power an Android
device (the THETA V) while using a USB OTG device (the Ethernet adaptor).

image::img/livestreaming/ethernet/oppkey-adapter.png[role="thumb" title="Cable to supply power to THETA V while it is using an USB OTG device"]

For the USB OTG to Ethernet, I am using the adapter below.

image::img/livestreaming/ethernet/ethernet-adapter.png[role="thumb" title="USB OTG Ethernet adapter"]

To be able to plug the USB OTG Ethernet adapter into the Y-cable, I needed to buy a microUSB female to USB male adapter. 

image::img/livestreaming/ethernet/microusb-to-usb.png[role="thumb" title="microUSB female to USB male adaptor to connect Y cable to USB OTG Ethernet"]


==== Troubleshooting the Ethernet Connection
When the THETA V is plugged into Ethernet, both of the LED 
lights on the USB OTG adapter need to be on.

image::img/livestreaming/ethernet/data-lights.jpeg[role="thumb" title="Verify LED lights on your Ethernet adapter"]


==== Other Hacks to Power the Camera 
...
video::dp5BLbtKVwk[youtube, width=660, height=480, title=RICOH THETA live streaming configuration of OBS]

video::Vxq-TJE_UR4[youtube, width=640, height=480, title=RICOH THETA live streaming to YouTube Live Events in 360]
